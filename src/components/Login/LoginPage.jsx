...
const LoginForm = (props) => {
	...
	const onSubmit = (values, action) => {
		...
		props.login(email, password, rememberMe, action);
	};
	...
	return (
		...
		>
			{(formik) => {
				const { errors, touched, isValid, status, submitForm } = formik;
				return (
					<Form>
						<div>
						...
						{status ? <div>Error: {status}</div> : null}
						<div>
							<button onClick={submitForm} type="submit" disabled={!isValid}>
								Log in
							</button>
						</div>
					</Form>
